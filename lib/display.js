  ...
  for (let i = 0; i < formatted.length; i++) {
    ...
    for (let row = 0; row < charDef.length; ++row) {
      for (let col = 0; col < charDef[row].length; ++col) {
        ...
        if (charDef[row][col] == 2) pixelColor = 0x00FF00
        if (charDef[row][col] == 3) pixelColor = 0x00A000
        if (charDef[row][col] == 4) pixelColor = 0xA0FF00
        if (charDef[row][col] == 5) pixelColor = 0x09FF00
