...
const FPS = Number(process.env.DISPLAY_FPS) || 60
const WIDTH = Number(process.env.DISPLAY_WIDTH) || 50
const HEIGHT = Number(process.env.DISPLAY_HEIGHT) || 5
const BRIGHTNESS = Number(process.env.DISPLAY_BRIGHTNESS) || 50
...
sm.setOnMessageCallback((options: any) => {
...
blocktime.setNewBlockCallback((blocktime: number) => {
...
let movingblock : typeof Movingblock | null = null
...
frontend.setActionCallback((data: any) => {
  switch (data.action) {
    case 'turnoff':
      state.running = false
      showDefault()
